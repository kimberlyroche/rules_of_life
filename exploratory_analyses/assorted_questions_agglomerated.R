# this file answers a couple of high-level questions about agglomerated data and presence/absence
# statistics and uniqueness of taxa
#
# at some point this should be discarded or cleaned up
...
for(level in levels) {
  ...
  png(file.path(relative_path,plot_dir,paste0("hist_",level,".png")))
  hist(indiv_appearances[indiv_appearances > 0], breaks=1:max(indiv_appearances))
  dev.off()
}
...
for(level in levels) {
  ...
  png(file.path(relative_path,plot_dir,paste0("hist_percent_",level,".png")))
  hist(indiv_percents, breaks=seq(0, 1, 0.01))
  dev.off()
}
