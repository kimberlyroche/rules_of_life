# this file is an old attempt at quantitatiely/visually assessing variation between replicates
# most have high (> 0.8) correlation but there are rare examples of significant disagreement
# between replicates
...
p <- ggplot(plot_data, aes(x)) +
  ...
  xlim(c(-1, 1))
p
ggsave(file.path(relative_path,plot_dir,"correlation_between_replicates.png"), scale=1.5, width=4, height=4, units="in")
...
p <- ggplot(plot_data, aes(x)) +
  ...
  xlim(c(-1, 1))
p
ggsave(file.path(relative_path,plot_dir,"correlation_across_replicates.png"), scale=1.5, width=4, height=4, units="in")
...
plot_timecourse_phyloseq(merged_samples, file.path(relative_path,plot_dir,paste0(individual,"_replicate_timecourse")), gapped=F, legend=T, legend_level="family")
