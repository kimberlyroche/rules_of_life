...
mean(dist_mat[which(labels == "baseline"), which(labels == "close")])
mean(dist_mat[which(labels == "close"), which(labels == "med")])
mean(dist_mat[which(labels == "med"), which(labels == "far")])
...
if(FALSE) {
# ape::PCoA; sanity check
embedding <- ape::pcoa(dist_mat, correction="none", rn=NULL) # rn = rownames 
df <- data.frame(x=embedding$vectors[,1], y=embedding$vectors[,2],
                 labels=c("baseline", rep("close", N),
                          "baseline", rep("med", N),
                          "baseline", rep("far", N)))
sum_eig <- sum(embedding$values$Eigenvalues)
cat("Var expl:",round(embedding$values$Eigenvalues[1]/sum_eig, 2)*100,"% x",round(embedding$values$Eigenvalues[2]/sum_eig, 2)*100,"%\n")
ggplot(df, aes(x=x, y=y, color=labels)) +
  geom_point(size=2)
}
