...
# usage: Rscript joint.R 10       { fit joint model with first k individuals }
...
if(do_save) {
  saveRDS(individual_start_end, paste0(output_dir,"joint_boundaries.rds"))
}
...
censor <- NULL
filename <- paste0(output_dir,"joint_censor.rds")
...
fit <- NULL
filename <- paste0(output_dir,"joint_fit.rds")
if(do_save & file.exists(filename)) {
...
Y.predicted <- NULL
filename <- paste0(output_dir,"joint_predict.rds")
#if(do_save & file.exists(filename)) {
...
if(predict_level == 1) {
  ...
  p <- ggplot(baboon.RMSE, aes(x=RMSE)) +
      geom_density()
  ggsave(paste0(output_dir,"evaljoint_errorplot_joint.png"), plot=p, dpi=200, units="in", width=15, height=10)
}
...
if(predict_level == 2) {
  ...
  ggsave(paste0(output_dir,"evaljoint_RMSE.png"), p, units="in", dpi=100, height=10, width=20)
}
