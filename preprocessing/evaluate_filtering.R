# this file applies and evaluates some low-abundance filtering options on sparsity of the resulting
# count table
...
local_filter <- function(data, count_threshold=100, sample_threshold=10, verbose=TRUE) {
...
# let's evaluate some filtering options
cts <- c(1, 3, 5, 10, 100)
...
for(count_threshold in cts) {
  for(sample_threshold in sts) {
    ...
    test <- local_filter(subsetted_data, count_threshold, sample_threshold, verbose=TRUE)
  }
}
