...
# usage: Rscript individual.R       { fit individual models with best-sampled individuals }
...
for(baboon in indiv_list) {
  ...
  if(predict_level == 2) {
    ...
    ggsave(paste0(output_dir,"evaljoint_errorplot_",baboon,".png"), p, units="in", dpi=100, height=3, width=10)
  }
}
...
if(predict_level == 1) {
  p <- ggplot(all.RMSE, aes(x=RMSE, color=baboon)) +
    geom_density()
  ggsave(paste0(output_dir,"evaljoint_individual_RMSE.png"), plot=p, dpi=200, units="in", width=15, height=10)
}
