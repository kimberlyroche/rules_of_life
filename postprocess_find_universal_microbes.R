...
ref <- readRDS("data/filtered_family_5_20.rds")
...
saveRDS(Sigmas, paste0(output_dir,"Sigmas.rds"))
saveRDS(correlations, paste0(output_dir,"correlations.rds"))

p <- ggplot(correlations, aes(value)) +
     geom_density(aes(color=meanclr)) +
     facet_wrap(~ taxon, ncol=10) +
     scale_color_gradient(low='blue', high='red')
ggsave(paste0(plot_dir,"universal_microbes.png"), plot=p, dpi=100, units="in", height=10, width=20)
