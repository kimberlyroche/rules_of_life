# visualize the effect of sample density on stray::basset fits by repeatedly
# downsampling a highly sampled individual and peaking at the element-wise 
# posterior mean taxonomic covariance

source("../include/R/general.R")
sourceCpp("../include/Rcpp/Riemann_dist.cpp")
...
for(ds in downsample_to) {
  ...
  png(paste0(plot_dir,"downsampled_meanSigma_",ds,".png"))
  image(meanSigma)
  dev.off()
  # repeat as correlation
  meanSigma <- cov2cor(meanSigma)
  png(paste0(plot_dir,"downsampled_meanSigma_corr_",ds,".png"))
  image(meanSigma)
  dev.off()
}
...
png(plot_dir,"downsampled_distance_distribution.png")
...
xlim <- c(min(all_d), max(all_d))
png(plot_dir,"downsampled_distance_distribution_corr.png")
