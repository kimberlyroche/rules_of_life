# simulate linearly adding noise to a base matrix (A)
# we'll do this by starting from random matrices A & B; A^TA and (A+B)^T(A+B) will give base and noise-added PSD matrices
# so long as we scale properly such that (1) A^TA and (A+B)^T(A+B) have approx. the same scale (this happens because we're
# effectively titrating in B on top of A at some proportion!)
