...
#baboons <- unique(read_metadata(filtered)$sname) # use all
#snames <- over_50; baboons <- snames[sample(length(snames))[1:5]] # use a few
baboons <- unique(read_metadata(filtered)$sname)
baboons <- baboons[sample(length(baboons))[1:100]] # use 100
for(b in 1:length(baboons)) {
  cat("Building kernels for baboon",baboons[b],"\n")
  baboon_counts <- subset_samples(filtered, sname==baboons[b])
...
it <- 1
components <- 7
estimates <- matrix(0, components, it)
for(i in 1:it) {
  ...
  res <- optim(par=runif(components), fn=ALT_logd_mat_t, method="L-BFGS-B",
