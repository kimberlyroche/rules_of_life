...
setwd('/gpfs/fs1/data/mukherjeelab/batch16s/dada2_pipeline/')
...
mapping <- "0_fixed_mapping/07_4994-17810_4662-17634_WI-031-T01_Mapping_File_Template_batch1r__MappingFile.tsv_corrected.txt"
filtpath <- "/gpfs/fs1/data/mukherjeelab/batch16s/dada2_pipeline/4_filter"
...
ps <- phyloseq(otu_table(seqtab, taxa_are_rows = FALSE),sample_data(map), tax_table(taxtab), DNAStringSet(refseq))
...
mult <- msa(refseq(ps), method="ClustalW", type='DNA', order='input', 
            verbose=FALSE)

# Use phangorn to construct tree 
# "Here we first construct a neighbor-joining tree, and then fit a 
#   GTR+G+I maximum likelihood tree using the neighbor-joining tree 
#   as a starting point."

phang.align <- as.phyDat(mult, type='DNA', names=names(refseq(ps)))
...
saveRDS(ps, 'phyloseq.rds') # save updated phyloseq object (overwrite previous)
saveRDS(fitGTR,'phylogeny.fitGTR.rds') # save full tree fit with parameters
write.tree(fitGTR$tree, file = 'phylo.nochim.tree') # write tree to newick file
