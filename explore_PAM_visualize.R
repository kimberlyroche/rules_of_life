# visualize selected PAM samples as a sanity check
...
for(ll in c("genus", "family", "phylum")) {
  ...
  for(k in kk) {
    ...
    samples <- read.table(file=paste0(output_dir,'PAM_selected_samples_',k,'.tsv'), sep='\t', header=TRUE, row.names=1)
    ...
    if(ll == "genus") { # just do this once
      ...
      ggsave(paste0(output_dir,"PAM_",ll,"_diagnostic_time_",k,".png"), p, units="in", dpi=200, height=10, width=20)
    }
    ...
    indiv_dist <- readRDS(paste0(output_dir,"saved_distance_Sigma_",ll,"_MAP.rds"))
    ...
    ggsave(paste0(output_dir,"PAM_",ll,"_diagnostic_dynamics_",k,"_1x2.png"), p, units="in", dpi=150, height=10, width=10)
    p <- ggplot(df) +
      geom_point(aes(x=x2, y=y2, color=selected), size=2) +
      scale_colour_manual(values = c("gray", "orange")) +
      xlab("PCoA 9 (Riemannian distance)") +
      ylab("PCoA 10 (Riemannian distance)")
    ggsave(paste0(output_dir,"PAM_",ll,"_diagnostic_dynamics_",k,"_9x10.png"), p, units="in", dpi=150, height=10, width=10)
    p <- ggplot(df) +
      geom_point(aes(x=x3, y=y3, color=selected), size=2) +
      scale_colour_manual(values = c("gray", "orange")) +
      xlab("PCoA 19 (Riemannian distance)") +
      ylab("PCoA 20 (Riemannian distance)")
    ggsave(paste0(output_dir,"PAM_",ll,"_diagnostic_dynamics_",k,"_19x20.png"), p, units="in", dpi=150, height=10, width=10)
    }
    ...
    if(ll == "family") { # just do this once
      ...
      ggsave(paste0(output_dir,"PAM_",ll,"_diagnostic_Aitchison_",k,".png"), p, units="in", dpi=150, height=10, width=10)
      }
      ...
      ggsave(paste0(output_dir,"PAM_",ll,"_diagnostic_UniFrac_",k,"_",retain_prop,"_1x2.png"), p, units="in", dpi=150, height=10, width=10)
