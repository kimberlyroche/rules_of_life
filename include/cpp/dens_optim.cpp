...
//' Optimize over the matrix-t density for variance component analysis
// [[Rcpp::export]]
double logd_matrixt(double s1, double s2, double s3, double s4, double s5, double s6, double s7, double s8, MatrixXd ilr_data,
                    MatrixXd week_kernel, MatrixXd season_kernel, MatrixXd group_kernel, MatrixXd age_kernel,
                    MatrixXd indiv_kernel, MatrixXd plate_kernel, MatrixXd conc_kernel, MatrixXd empty_kernel) {
  double N = ilr_data.cols();
  double P = ilr_data.rows();
  double upsilon = P + 10;
  MatrixXd K = MatrixXd::Identity(P,P);
  // constrains imposed in optim()
  MatrixXd A = s1*week_kernel + s2*season_kernel + s3*group_kernel + s4*age_kernel +
    s5*indiv_kernel + s6*plate_kernel + s7*conc_kernel + s8*empty_kernel;
