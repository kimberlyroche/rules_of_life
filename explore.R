...
if(task == 6) {
  if(FALSE) {
    # test - how do two individuals, ordinated and plotted look?
    baboon_counts <- subset_samples(filtered, sname %in% c("DUI", "DAS"))
    # PCoA ordination using Bray-Curtis distance
    ord <- ordinate(baboon_counts, "PCoA", "bray")
    cat("Plotting test ordination (ACA + DUI)...\n")
    p <- plot_ordination(baboon_counts, ord, type="sample", color="sname")
    ggsave("test_ordination.png", plot=p, width=6, height=5, units="in", scale=1.5)
  }
...
#  ggsave("matgrp_ordination_100.png", plot=p, width=6, height=5, units="in", scale=1.5)
  ...
  ggsave("age_ordination_100.png", plot=p, width=6, height=5, units="in", scale=1.5)
