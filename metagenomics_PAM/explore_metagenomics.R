# this essentially renders barplots for simulated (Piphillin) metagenomics data for
# a handful of well-sampled individuals

relative_path <- ".."

source(file.path("include/R/general.R"))
source(file.path("include/R/metagenomics"))
...
parsed_filename <- file.path(relative_path,data_dir,"Piphillin_20190222/parsed_enzymes.RData")
if(file.exists(parsed_filename)) {
  ...
  save(data.piphillin, file=file.path(data_dir,"parsed_enzymes.RData")
}
